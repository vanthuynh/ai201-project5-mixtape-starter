...
**How you reproduced it:** I asked Claude to help troubleshoot `search_songs()`. Claude ran the raw SQL generated by the query against a test song with 3 tags and confirmed the `LEFT OUTER JOIN` to `song_tags` produced 3 rows for that single song — even though the existing `tests/test_search.py` tests were passing.

**How you found the root cause:** I ran all test cases and they all passed. Then I asked Claude to test SQL query by comparing the raw SQL row count (3 rows) against the ORM's `.all()` result (1 row) and found that SQLAlchemy's legacy `Query` API auto-deduplicates full-entity results by primary key — which is why the existing tests happened to pass despite the query being wrong. The `song_tags` join was never actually used for filtering (the `WHERE` clause only checks `title`/`artist`), so it existed purely to multiply rows with no benefit.

**What was your navigation path?** `/services` module --> `search_service.py` --> `search_songs()`

**The root cause** `search_songs()` joined `Song` to `song_tags` with `.outerjoin(song_tags, Song.id == song_tags.c.song_id)` but never referenced any `song_tags` column in the filter. Each tag associated with a song added an extra joined row, so a song with N tags produces N duplicate rows at the SQL level — masked here only by an implicit ORM dedup behavior, not a real fix.

**Your fix and side-effect check**: removed the `.outerjoin(song_tags, ...)` entirely, since tags are already loaded independently via the `Song.tags` relationship used inside `song.to_dict()`. Re-ran `tests/test_search.py` (5/5 passing) and the full suite (`pytest tests/`, 13/13 passing) to confirm no regressions, and manually confirmed `to_dict()['tags']` still returns the full tag list after the join was removed.

### Issue #4 - I got notified when a friend added my song to a playlist but not when they rated it

**How you reproduced it:** I compared `add_to_playlist()` and `rate_song()` side by side — both live in `notification_service.py` and both represent "a friend interacted with a song I shared." `add_to_playlist()` ends by calling `create_notification()`; `rate_song()` saves the `Rating` row and commits, but never calls `create_notification()` at all, so no notification is ever produced for a rating.

**How you found the root cause:** Since both actions are meant to notify the original sharer, I checked whether `rate_song()` contained the same "notify `song.shared_by` unless they did it themselves" logic that `add_to_playlist()` has. It didn't — the notification call was simply missing.

**What was your navigation path?** `routes/songs.py: rate()` --> `notification_service.py: rate_song()`

**The root cause** `rate_song()` never called `create_notification()` after saving the `Rating`, unlike `add_to_playlist()`, which fires a notification for the equivalent "someone interacted with your shared song" event.

**Your fix and side-effect check**: added a `song.shared_by != user_id` guard followed by `create_notification(user_id=song.shared_by, notification_type="song_rated", body=f"{rater.username} rated your song '{song.title}' {score}/5.")` right after the rating is committed, mirroring `add_to_playlist()`'s pattern exactly. Side effect checked: rating your own shared song does **not** notify yourself, same guard as the playlist case.
